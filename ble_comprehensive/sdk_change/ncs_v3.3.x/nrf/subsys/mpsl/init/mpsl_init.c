...
/* 应用可重定向 RADIO / 定时器中断（例如 Direct Test Mode 需要独占 radio）。
 * 弱定义默认返回 false（不重定向），不影响其它应用的既有行为。
 * 返回 true 表示该中断已由应用处理，MPSL 不再处理。
 */
__weak bool mpsl_radio_isr_redirect(void)
{
	return false;
}

__weak bool mpsl_timer_isr_redirect(void)
{
	return false;
}
