...
#ifndef NETCPU_APP_SLOT_OFFSET
#if DT_NODE_EXISTS(DT_NODELABEL(s0_partition))
#define NETCPU_APP_SLOT_OFFSET  DT_REG_ADDR(DT_NODELABEL(s0_partition))
#define NETCPU_APP_SLOT_SIZE    DT_REG_SIZE(DT_NODELABEL(s0_partition))
#else
/*
 * When s0_partition is not visible from the local DTS (common for
 * app-core MCUboot builds), hardcode the defaults for the known
 * nRF5340 partition layout: s0_partition at flash1 offset 0x8800,
 * flash1 base 0x01000000 => absolute address 0x01008800.
 * These must match the actual s0_partition layout defined in the
 * b0n/ipc_radio DTS overlays. Override via -D compiler flags if
 * a different layout is required.
 */
#define NETCPU_APP_SLOT_OFFSET  0x01008800
#define NETCPU_APP_SLOT_SIZE    0x00037800
#endif
#endif
#ifndef NETCPU_APP_SLOT_END
