...
extern struct bt_conn *conn_id[CONFIG_BT_MAX_CONN];
extern struct k_sem sem_conn_type;
...
void BLEManagerImpl::DriveBLEState()
{
    ...
    )
    {
        ...
        {
            // ChipLogProgress(DeviceLayer, "DriveBLEState -> restart adv");
    ...
    else
    {
        ...
        {
            // ChipLogProgress(DeviceLayer, "DriveBLEState -> stop adv");
            err = StopAdvertising();
            if (err != CHIP_NO_ERROR)
            {
        ...
        {
            // ChipLogProgress(DeviceLayer, "DriveBLEState -> unreg service");
...
inline CHIP_ERROR BLEManagerImpl::PrepareAdvertisingRequest()
{
    ...
    {
        // ChipLogProgress(DeviceLayer, "slow adv param");
...
CHIP_ERROR BLEManagerImpl::StartAdvertising()
{
    ...
    if (!mFlags.Has(Flags::kAdvertising))
    {
        ...
        if (mFlags.Has(Flags::kFastAdvertisingEnabled))
        {
            // ChipLogProgress(DeviceLayer, "start slow timer");
...
CHIP_ERROR BLEManagerImpl::HandleGAPConnect(const ChipDeviceEvent * event)
{
   ...
   if (conn_id[1] != connEvent->BtConn)
    {
        bt_conn_unref(connEvent->BtConn);
        ChipLogProgress(DeviceLayer, "Not Matter BLE %p VS %p", conn_id[1], connEvent->BtConn);
        //if Matter BLE connection is not established, return directly
        return CHIP_NO_ERROR;
    } 
