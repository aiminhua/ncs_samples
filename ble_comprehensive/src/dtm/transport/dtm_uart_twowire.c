...
static uint16_t dtm_cmd_put(uint16_t cmd)
{
	...
	switch (cmd_code) {
	case LE_TEST_SETUP:
		printk("Executing test setup command. Control: %d Parameter: %d\n", control,
											parameter);
		return on_test_setup_cmd(control, parameter);

	case LE_TEST_END:
		printk("Executing test end command. Control: %d Parameter: %d\n", control,
											parameter);
		return on_test_end_cmd(control, parameter);

	case LE_RECEIVER_TEST:
		printk("Executing reception test command. Channel: %d\n", chan);
		return on_test_rx_cmd(chan);

	case LE_TRANSMITTER_TEST:
		printk("Executing transmission test command. Channel: %d Length: %d Type: %d\n",
										chan, length, type);
		return on_test_tx_cmd(chan, length, type);

	default:
		printk("Received unknown command code %d\n", cmd_code);
		return LE_TEST_STATUS_EVENT_ERROR;
	}
}
...
union dtm_tr_packet dtm_tr_get(void)
{
	...
	for (;;) {
		...
		if (current_time > (msb_time + MAX_ITERATIONS_NEEDED_FOR_NEXT_BYTE)) {
			...
			msb_time = current_time;
			/* Go back and wait for 2nd byte of command word. */
			printk("Received byte discarded\n");
			continue;
		} else {
			dtm_cmd |= rx_byte;
			printk("Received 0x%04x command\n", dtm_cmd);
			tmp.twowire = dtm_cmd;
			return tmp;
		}
	}
...
int dtm_tr_process(union dtm_tr_packet cmd)
{
	...
	printk("Processing 0x%04x command\n", tmp);
